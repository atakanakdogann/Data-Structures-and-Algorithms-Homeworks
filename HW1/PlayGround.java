...
public class PlayGround implements basicInterface{
    //PlayGround Array
    private int[][] arr;
    // Building Array. I decided maximum 10 building for street
    private Buildings[] buildings = new Buildings[10];
    ...
    public void driverMethod(){
        ...
        buildings[buildingCount].setSide(0);    buildings[buildingCount].setPosition(10);
        buildings[buildingCount].setLength(12); buildings[buildingCount].setHeight(17);
        buildings[buildingCount].setOwner("Atakan Akdogan");    buildings[buildingCount].setColor("Green"); buildings[buildingCount].setRoomNumber(3);
        //Fills PlayGround Array
        for(int i = 30 - buildings[buildingCount].getHeight(); i < 30; i++){
            for(int j = buildings[buildingCount].getPosition()-(buildings[buildingCount].getLength()/2); j < buildings[buildingCount].getPosition()+(buildings[buildingCount].getLength()/2); j++){
                if(arr[i][j] == 1){
                    arr[i][j] = 3;
                }
                else
        ...
        buildingCount++;
        //ADD OFFICE
        buildings[buildingCount] = new Office();
        buildings[buildingCount].setSide(1);    buildings[buildingCount].setPosition(15);
        buildings[buildingCount].setLength(20); buildings[buildingCount].setHeight(13);
        buildings[buildingCount].setOwner("Michael Scott");    buildings[buildingCount].setJobType("Paper Company");
        for(int i = 30 - buildings[buildingCount].getHeight(); i < 30; i++){
            for(int j = buildings[buildingCount].getPosition()-(buildings[buildingCount].getLength()/2); j < buildings[buildingCount].getPosition()+(buildings[buildingCount].getLength()/2); j++){
                if(arr[i][j] == 1){
                    arr[i][j] = 3;
        ...
        buildingCount++;
        //ADD MARKET
        buildings[buildingCount] = new Market();
        buildings[buildingCount].setSide(0);    buildings[buildingCount].setPosition(24);
        buildings[buildingCount].setLength(14); buildings[buildingCount].setHeight(9);
        buildings[buildingCount].setOwner("Mr Bim");    buildings[buildingCount].setJobTimes("08.30", "22.00");
        for(int i = 30 - buildings[buildingCount].getHeight(); i < 30; i++){
            for(int j = buildings[buildingCount].getPosition()-(buildings[buildingCount].getLength()/2); j < buildings[buildingCount].getPosition()+(buildings[buildingCount].getLength()/2); j++){
                if(arr[i][j] == 1){
                    arr[i][j] = 3;
        ...
        if(pos>=street_size){
            ...
            while(true){
                ...
                if(tempStr.equalsIgnoreCase("House")){
                    ...
                    tempStr = sc.nextLine();
                    buildings[buildingCount].setOwner(tempStr);
                    System.out.println("Please enter Color of House");
                    tempStr = sc.nextLine();
                    buildings[buildingCount].setColor(tempStr);
                    System.out.println("Please enter Room Number of House");
                    temp = sc.nextInt();
                    buildings[buildingCount].setRoomNumber(temp);
                    break;
                }
                    ...
                    tempStr = sc.nextLine();
                    buildings[buildingCount].setJobType(tempStr);
                    break;
                }
                    ...
                    buildings[buildingCount].setJobTimes(tempStr, tempStr2);
                    break;
                }
            }
            buildings[buildingCount].setSide(side);
            buildings[buildingCount].setPosition(pos);
            buildings[buildingCount].setLength(len);
            buildings[buildingCount].setHeight(hei);
            buildingCount++;
        }
    ...
    public void deleteBuilding()throws Exception{
        ...
        for(int i=0;i<buildings.length;++i){
            if(buildings[i].equals(buildings[temp])){
                 flag=true;
                 break;
            }
        }
        //Cleans from map
        for(int i = 30 - buildings[temp].getHeight(); i < 30; i++){
            for(int j = buildings[temp].getPosition()-(buildings[temp].getLength()/2);j < buildings[temp].getPosition()+(buildings[temp].getLength()/2);j++){
                if(arr[i][j] == 3){
                    arr[i][j] = 1;
                }
                else if(arr[i][j] == 1){
                    arr[i][j] = 0;
                    arr[30+buildings[temp].getSide()][j] = 2;
                }
